# Aim : To measure changes in storage wrt time by inserting constant number of packets per second
...
# Argument 7: Packet Insertion Rate
...
do
    rm -f $DIRECTORY
    numPackets=$((interval*packetInsertionRate))
    # Run the experiment
    echo "Interval $interval starts at $(date +"%T")" >> "$4/../timer.log"
    nohup ./waf --run "$1 --hostPairs=$2 --packetNum=$numPackets --storePath=$4/ --pathLength=$5 --dataSize=$6 --stopTime=$interval --mode=${11}"
