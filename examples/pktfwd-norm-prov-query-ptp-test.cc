...
#include "ns3/pktfwd-norm-prov-query-module.h"
...
#define linkhr(host, sw)\
  tuple(PktfwdNormProvQuery::LINKHR,\
...
#define link(sw, nei)\
  tuple(PktfwdNormProvQuery::LINK,\
...
#define initpacket(host, srcadd, dstadd, data)               \
  tuple(PktfwdNormProvQuery::INITPACKET,\
...
#define flowentry(sw, dst, next)		\
  tuple(PktfwdNormProvQuery::FLOWENTRY,\
...
#define GRAPHIC_PREFIX "/localdrive1/harshal/bandwidth/pktfwd_nocomp_bandwidth_graphics/"
...
using namespace ns3::rapidnet::pktfwdnormprovquery;
...
void Print ()
{
  PrintRelation (apps, PktfwdNormProvQuery::RECVPACKET);
  PrintRelation (apps, PktfwdNormProvQuery::RULEEXEC);
...
{
  LogComponentEnable("PktfwdNormProvQuery", LOG_LEVEL_INFO);
  LogComponentEnable("RapidNetApplicationBase", LOG_LEVEL_INFO);

  uint32_t hostPairs = 1000;
  string storePath = "/localdrive1/harshal/bandwidth/pktfwd_nocomp_storage/";
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<PktfwdNormProvQueryHelper> ();
