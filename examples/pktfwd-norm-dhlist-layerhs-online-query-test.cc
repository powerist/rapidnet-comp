...
#include "ns3/pktfwd-norm-dhlist-layerhs-online-query-module.h"
...
#define link(sw, nei)\
  tuple(PktfwdNormDhlistLayerhsOnlineQuery::LINK,\
...
#define initpacket(host, srcadd, dstadd, data)               \
  tuple(PktfwdNormDhlistLayerhsOnlineQuery::INITPACKET,\
...
#define flowentry(sw, dst, next)		\
  tuple(PktfwdNormDhlistLayerhsOnlineQuery::FLOWENTRY,\
...
#define programID(node, dst, ID)\
  tuple(PktfwdNormDhlistLayerhsOnlineQuery::PROGRAMID,\
...
using namespace ns3::rapidnet::pktfwdnormdhlistlayerhsonlinequery;
...
void Print ()
{
  PrintRelation (apps, PktfwdNormDhlistLayerhsOnlineQuery::RECVPACKET);
  PrintRelation (apps, PktfwdNormDhlistLayerhsOnlineQuery::RULEEXEC);
  PrintRelation (apps, PktfwdNormDhlistLayerhsOnlineQuery::PROVHASHTABLE);  
}
...
void SchedulePacketTrans(int totalNum, int totalSwcNum, int hostPairs, int packetNum, int dataSize, int pathLength, map<int, int> rtables[MAX_NODE_NUM])
{
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue("packetNum", "Number of packets sent between each pair of hosts", packetNum);  
    cmd.AddValue("dataSize", "Payload Data Size", dataSize);
        cmd.AddValue("pathLength", "Length of Path", pathLength);  
  cmd.Parse(argc, argv);
  ...
  // Schedule traffic
  SchedulePacketTrans(totalNum, totalSwcNum, hostPairs, packetNum,dataSize, pathLength,rtables);
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<PktfwdNormDhlistLayerhsOnlineQueryHelper> ();
