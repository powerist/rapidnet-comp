...
#include "ns3/pktfwd-norm-prov-comp-strawman-ete-module.h"
...
#define linkhr(host, sw)\
  tuple(PktfwdNormProvCompStrawmanEte::LINKHR,\
...
#define link(sw, nei)\
  tuple(PktfwdNormProvCompStrawmanEte::LINK,\
...
#define initpacket(host, srcadd, dstadd, data)               \
  tuple(PktfwdNormProvCompStrawmanEte::INITPACKET,\
...
#define flowentry(sw, dst, next)		\
  tuple(PktfwdNormProvCompStrawmanEte::FLOWENTRY,\
...
using namespace ns3::rapidnet::pktfwdnormprovcompstrawmanete;
...
void Print ()
{
  PrintRelation (apps, PktfwdNormProvCompStrawmanEte::RECVPACKET);
  PrintRelation (apps, PktfwdNormProvCompStrawmanEte::RULEEXEC);
  //PrintRelation (apps, PktfwdNormProvCompStrawmanEte::PROVHASHTABLE);  
}
...
{
  LogComponentEnable("PktfwdNormProvCompStrawmanEte", LOG_LEVEL_INFO);
  LogComponentEnable("RapidNetApplicationBase", LOG_LEVEL_INFO);

  uint32_t hostPairs = 1000;
  string storePath = "/localdrive1/harshal/pktfwd_prov_storage_strawman_ete/";
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<PktfwdNormProvCompStrawmanEteHelper> ();
