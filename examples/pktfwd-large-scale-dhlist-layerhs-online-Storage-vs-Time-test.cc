...
void SetupProgramID(int switchNum, int src, int dst, map<int,int> rtables[MAX_NODE_NUM])
{
  srand(0);
  int tempDst = src;
  //while(tempDst != dst)
    {
      ...
      insert_programID(tempSrcInserted,tempDstInserted,progID.c_str());
    }
...
void SchedulePacketTrans(int totalNum, int totalSwcNum, int hostPairs, int packetNum, int pathLength, map<int, int> rtables[MAX_NODE_NUM],int dataSize, int *src, int *dst)
{
  ...
  double trigger_time = PACKET_INIT;
  int dataCount = 0;
  srand(1);
  //hostPairs =1;
  //int srcArray[] = {0,0,55};
...
main (int argc, char *argv[])
{
  ...
  Simulator::Schedule (Seconds(5.0000), SetupProgramID, totalSwcNum, src, dst, rtables);  
  ...
  apps.Stop (Seconds (PACKET_INIT+stopTime));

  Simulator::Schedule (Seconds(PACKET_INIT+stopTime-1), SerializeProv, totalNum, storePath);  
