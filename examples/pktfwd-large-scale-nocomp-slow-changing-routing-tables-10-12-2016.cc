			 ...
			 int pathLength, map<int, int> rtables[MAX_NODE_NUM],int dataSize, int *src, int *dst)
{
  ...
  double trigger_time = PACKET_TIME;
  int dataCount = 0;
  srand(1);
  //hostPairs =1;
  //int srcArray[] = {0,0,55};
...
void findUnConnectedPairs(vector< pair<int,int> >& nonConnectedPairs, map<int,int> rtables[],int table_size)
{
...
void updateFlowTable(int src, int dst, int next)
{
  insert_flowentry(src, dst, next);
}
  ...
  for(vector< pair<int,int> >::iterator iter = nonConnectedTables.begin(); iter != nonConnectedTables.end() && counter>0; iter++, counter--)
    {
      ...
      Simulator::Schedule (Seconds (timer),updateFlowTable,src,dst,next);
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue("dataSize","Data size of packet",dataSize);
  ...
  /*
    Extract all pairs of nodes that are not directly connected from the rtables
   */

  vector< pair<int,int> > unConnectedPairs;

  findUnConnectedPairs(unConnectedPairs,rtables,MAX_NODE_NUM);

  int numEntries = stopTime/TIME_STEP_FLOW_ENTRY;

  changeFlowEntries(unConnectedPairs,numEntries);
   // Schedule traffic
  int src,dst;
  if(packetNum == -1)
  ...
  cout<<"STOP TIME "<<PACKET_TIME+stopTime<<endl;
  apps.Stop (Seconds (PACKET_TIME+stopTime));

  Simulator::Schedule (Seconds(PACKET_TIME+stopTime-1), SerializeProv, totalNum, storePath);  
