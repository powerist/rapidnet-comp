...
void UpdateTableRandomTree(int numNodes, int minFanout, int maxFanout, int numRequests, int numURL)
{
  int nextNodeID = 2;
  int rootNodeID = 2;
  string path = ".";
  ...
  //list<string>::iterator iter = pathList.begin();
   ...
   list<string> pathListReduced;
  for(list<string>::iterator iter1 = pathList.begin(); iter1!=pathList.end() && numURL >0;iter1++, numURL--)
    {
      pathListReduced.push_back(*iter1);
    }

  int resultNodeID = 1;
  int counter = 0;
  double timer = 0;
  insertRandomURL(pathListReduced,numRequests);
}
...
int main(int argc,char *argv[])
{
  ...
  cmd.AddValue("numURL","Number of URLs",numURL);
  cmd.Parse(argc,argv);
  ...
  if(numNodes == -1)
    ...
    Simulator::Schedule(Seconds(0.01),UpdateTableRandomTree,numNodes,fanout,fanout+1,numRequests,numURL);
