...
void SchedulePacketTrans(int totalNum, int totalSwcNum, int hostPairs, int packetNum,int dataSize,   map<int, int> rtables[MAX_NODE_NUM])
{
  ...
  cout<<"Average path length "<<avgLength*1.0/hostPairs;
}
...
main (int argc, char *argv[])
{
  ...
  // Schedule traffic
  SchedulePacketTrans(totalNum, totalSwcNum, hostPairs, packetNum,dataSize, rtables);
