...
#include "ns3/pktfwd-norm-dist-hlist-rmitm-query-module.h"
...
#define link(sw, nei)\
  tuple(PktfwdNormDistHlistRmitmQuery::LINK,\
...
#define initpacket(host, srcadd, dstadd, data)               \
  tuple(PktfwdNormDistHlistRmitmQuery::INITPACKET,\
...
#define flowentry(sw, dst, next)		\
  tuple(PktfwdNormDistHlistRmitmQuery::FLOWENTRY,\
...
void Print ()
{
  PrintRelation (apps, PktfwdNormDistHlistRmitmQuery::RECVPACKET);
  PrintRelation (apps, PktfwdNormDistHlistRmitmQuery::RULEEXEC);
  PrintRelation (apps, PktfwdNormDistHlistRmitmQuery::PROV);
}
...
{
  LogComponentEnable("PktfwdNormDistHlistRmitmQuery", LOG_LEVEL_INFO);
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<PktfwdNormDistHlistRmitmQueryHelper> ();
