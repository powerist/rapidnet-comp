...
#include "ns3/pktfwd-norm-prov-comp-online-ete-module.h"
...
#define link(sw, nei)\
  tuple(PktfwdNormProvCompOnlineEte::LINK,\
...
#define initpacket(host, srcadd, dstadd, data)               \
  tuple(PktfwdNormProvCompOnlineEte::INITPACKET,\
...
#define flowentry(sw, dst, next)		\
  tuple(PktfwdNormProvCompOnlineEte::FLOWENTRY,\
...
void Print ()
{
  PrintRelation (apps, PktfwdNormProvCompOnlineEte::RECVPACKET);
  PrintRelation (apps, PktfwdNormProvCompOnlineEte::RULEEXEC);
  PrintRelation (apps, PktfwdNormProvCompOnlineEte::PROVHASHTABLE);  
}
...
main (int argc, char *argv[])
{
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<PktfwdNormProvCompOnlineEteHelper> ();
